...
pub(in crate::lower) fn post_lower_var_name<'a>(
    il: &Il,
    interner: &'a Interner,
    node: NodeId,
) -> Option<&'a str> {
...
pub(in crate::lower) fn post_lower_unshadowed_symbol_evidence_id(
    il: &mut Il,
    node: NodeId,
    expected: &str,
) -> Option<EvidenceId> {
...
pub(in crate::lower) fn post_lower_qualified_global_symbol_evidence_id(
    il: &Il,
    node: NodeId,
    path: &str,
) -> Option<EvidenceId> {
...
pub(in crate::lower) fn post_lower_static_global_call_parts<'a>(
    il: &Il,
    interner: &'a Interner,
    call: NodeId,
...
pub(in crate::lower) fn post_lower_static_global_callee_parts<'a>(
    il: &Il,
    interner: &'a Interner,
    callee: NodeId,
...
pub(in crate::lower) fn post_lower_imported_binding_symbol_evidence_id(
    il: &mut Il,
    interner: &Interner,
    node: NodeId,
    module: &str,
...
pub(in crate::lower) fn post_lower_imported_namespace_symbol_evidence_id(
    il: &mut Il,
    interner: &Interner,
    node: NodeId,
    module: &str,
...
pub(in crate::lower) fn post_lower_binding_symbol_evidence_id(
    il: &Il,
    interner: &Interner,
    node: NodeId,
...
pub(in crate::lower) fn post_lower_java_wildcard_import_evidence_id(
    il: &Il,
    interner: &Interner,
    module: &str,
    simple_type: &str,
...
pub(in crate::lower) fn post_lower_explicit_import_conflicts(
    il: &Il,
    _interner: &Interner,
    module: &str,
    simple_type: &str,
...
pub(in crate::lower) fn post_lower_required_module_evidence_id(
    il: &mut Il,
    interner: &Interner,
    module: &str,
    use_span: Span,
...
pub(in crate::lower) fn post_lower_require_call_callee_if_matches(
    il: &Il,
    interner: &Interner,
    call: NodeId,
    module_hash: u64,
...
pub(in crate::lower) fn post_lower_top_level_statements(il: &Il) -> Vec<NodeId> {
...
pub(in crate::lower) fn post_lower_source_call_evidence_id(
    il: &Il,
    node: NodeId,
    call: SourceCallKind,
) -> Option<EvidenceId> {
...
pub(in crate::lower) fn post_lower_has_python_wildcard_import_evidence(il: &Il) -> bool {
    il.evidence.iter().any(|record| {
        record.status == EvidenceStatus::Asserted
            && matches!(
                record.kind,
...
pub(in crate::lower) fn post_lower_file_defines_name_visible_at(
    il: &Il,
    interner: &Interner,
    name: &str,
...
pub(in crate::lower) fn post_lower_unit_defines_name(
    il: &Il,
    interner: &Interner,
    name: &str,
    occurrence_span: Span,
