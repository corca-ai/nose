...
- `labels/prune_manifest.json` records the file-level prune audit: removed generated
  ...
  digest. The checked-in manifest covers the active v6 labels and current 120-repository
  pinned corpus; reconstructing the corpus must reproduce it.
...
`labels/refactoring_families.v6.json` is the active hash-checked composite product gold
set: the frozen 9,461-family v5 recall/precision base plus 115 split-safe current-top-10
precision labels, for 9,576 families over all 120 repositories. It was built by a
...
product labels for both corpus splits.
