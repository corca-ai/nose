...
| `hof.filter-map.drop-condition-coordinate` | `modeled-controlled` | `focused-executable`, `source-evidence` | Only proves that both surfaces drop the same source elements; the Swift slice requires a direct, attribute- and modifier-free function parameter with language-core bracket-array (`[T]`) evidence and the callback parameter itself as the condition. |
| `hof.filter-map.emitted-value-coordinate` | `modeled-controlled` | `focused-executable`, `source-evidence` | Only proves that present branches, maps, or guarded pushes emit the same value; the Swift slice re-emits that same callback parameter from the proven `[T]` source. |
...
| `hof.flat-map.nested-iteration-order` | `modeled-controlled` | `focused-executable`, `source-evidence` | Only proves matching outer/inner traversal coordinates; Swift requires direct language-core plain bracket-array outer/inner sources and one direct collection or admitted inner map. |
| `hof.flat-map.emitted-value-coordinate` | `modeled-controlled` | `focused-executable`, `source-evidence` | Only proves each matched nested coordinate emits the same value; the Swift slice retains the admitted inner-map contribution in the FlatMap graph. |
| `collection.flatten-depth.one-level` | `modeled-controlled` | `focused-executable`, `source-evidence` | Only closes flattened-vs-nested output shape boundaries; recursive/derived/scalar Swift callback results and custom dispatch remain closed. |
...
|---|---|---|---|---|---|
| `effect.pure-callback` | modeled-controlled for exact Array unary inline map/filter/flatMap callbacks with one plain parameter and immutable local/captured reads; value-transform operators require exact primitive non-dispatch proof, `instanceof` remains closed, and abstract integers do not prove Number | modeled-controlled for ordered Array/Collection unary inline map/filter/reject blocks; Ruby operators, map-key hashing, and splat surfaces remain closed | modeled-controlled for ordered Array/Collection unary inline map/filter/flatMap callbacks; compactMap is limited to a direct language-core-proven plain, attribute- and modifier-free bracket-array (`[T]`) function parameter with the same callback parameter as Bool condition and emitted value in a closed nil-literal namespace; controlled aggregate flatMap additionally permits at most one admitted pure filter over each direct bracket-array outer/inner source before one direct collection or admitted inner map, while source-independent map contributions, recursive/derived/scalar outputs, imports, custom dispatch, and effects remain closed | modeled-controlled for the `java.util.Arrays`-backed Stream flatMap/filter/map plus explicit-reduce slice; emitted contributions retain outer/inner coordinates, while source-independent maps, repeated/derived same-source aliases, recursive depth, changed sources, custom dispatch, and callback effects remain closed | modeled fact; lazy transform attachment is separate from current effect-preserving iterator admission |
| `effect.pure-predicate` | separately modeled for admitted quantifier predicates | separately modeled for Enumerable quantifier blocks | separately modeled for eager allSatisfy and controlled outer/inner flatMap aggregate filters; custom dispatch and effectful predicates remain closed | separately modeled for controlled Stream outer/inner flatMap aggregate filters; changed coordinates, custom dispatch, and effects remain closed | separately modeled for Iterator any/all predicates |
...
| `reduction.terminal-predicate-coordinate` | open int-bool terminal proof | open | open | modeled via quantifier card | modeled-controlled TS any/some plus dense-literal one-arg TS/JS every | modeled-controlled any/all | open receiver proof | open |
| `reduction.short-circuit-direction` | open int-bool terminal proof | open | open | modeled via quantifier card | modeled-controlled TS any/some plus dense-literal one-arg TS/JS every; array-param, callback-extra-arg, and JS prototype-replacement every stay split | modeled-controlled any/all | open receiver proof | open |
