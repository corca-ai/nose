...
- **blocked by**: the current fzf member has no modeled bound-order evidence; the focused Go proof boundary verifies that parameter naming such as `Constrain(val, minimum, maximum)` is not a proof, the current boltons/fzf pair has no shared integer-only domain proof; boltons documents mixed int/float/comparable values and Go `cmp.Ordered` remains a float/NaN-sensitive boundary
- **notes**: The proof-backed integer Clamp canon now covers min/max composition plus controlled two-comparison, library method, and Go typed integer bridge surfaces when literal or asserted Guard(BoundOrder) evidence proves lo<=hi and integer-domain evidence excludes float/NaN behavior. The linked boltons/fzf real pair remains routed proof-fact-prerequisite with an executable split expectation because the real fzf member lacks modeled order evidence and the cross-language pair lacks a shared integer-only domain proof.
...
- **owner route**: `team-a-detector` (#756/#782) · evidence tier: `frontier-recorded` · cost `low` · risk `low` · substrate `none`
...
- **real frontier replay**: `string-affix-prefix-focused-controlled-pair`, `string-affix-suffix-focused-controlled-pair`, `string-affix-parameter-coordinate-controlled-pair`, `string-affix-ruby-prefix-controlled-pair`, `string-affix-swift-prefix-focused-controlled-pair`, `string-affix-swift-suffix-focused-controlled-pair`, `string-affix-swift-suffix-parameter-coordinate-controlled-pair`, `string-affix-swift-literal-binding-controlled-pair` (`real_frontier_replay.v1.json`)
- **representative locations**:
  ...
  - `nose` (focused, Swift) `crates/nose-cli/tests/fixtures/string_affix_550/prefix.swift:1-3`
  - `nose` (focused, Swift) `crates/nose-cli/tests/fixtures/string_affix_552/local_binding_swift.swift:1-4`
- **current detector result (primary linked evidence)**: miss=False · `nose` @ `#756 semanti` — Semantic queries report the expected prefix, suffix, Ruby prefix/suffix, same-role prefix/suffix parameter, and immutable binding families including focused Swift witnesses while adjacent hard negatives stay outside those families.
...
- **why now**: string_prefix_suffix has broad controlled coverage across core languages, focused Swift hasPrefix/hasSuffix evidence, and closeout evidence for Go ownership, Ruby receiver proof, and affix-coordinate boundaries. The remaining value is to preserve the receiver/API/affix/direction/arity perimeter as reusable neutral facts before future case-insensitive, locale, offset, or multi-affix expansions add more surfaces.
- **blocked by**: nothing
...
- **owner route**: `team-a-detector` (#758/#784/#783/#797) · evidence tier: `frontier-recorded` · cost `medium` · risk `medium` · substrate `fragment-contract`
- **breadth**: repo 88% · primary-language 100% (8/8) · dev 58 · held-out 47 · both-splits
- **semantic claim**: Reduction surfaces can converge only when source evidence proves the same traversal source, identity and empty-input behavior, numeric.aggregate-value-model-domain for controlled aggregate arithmetic, numeric.selection-value-order-domain for controlled min/max or relational selection, reduction step or terminal predicate coordinate, any/all short-circuit direction, selection seed/domain behavior for min/max, receiver/API identity for protocol methods, and effect-safe value-only predicates or callbacks. Flat-map aggregate convergence additionally requires a proven one-level outer/inner stream, emitted-value and source-cardinality coordinates, distinct outer/inner/terminal guard coordinates, reducer steps and terminal predicates that consume the flattened value, and standard dispatch. The focused slice admits sum loops and typed reduce/sum APIs across C, Go, Java, JS loop, Python, Rust, and TypeScript; Rust any/all plus focused Iterator::all universal terminal bridges; TypeScript any/some and dense-literal one-argument TypeScript/JavaScript every/for-of terminal bridges; Swift eager Array/Collection allSatisfy terminal bridges; controlled Java Stream flatMap/reduce nested-reduction bridges; controlled Swift flatMap/allSatisfy nested-counterexample-loop bridges; and Python/Rust seeded min/max selection loops/folds. Wrong seeds, changed additive/product/count steps, flat-map guards, sources, source-independent maps, aliased same/derived source coordinates, direct or wrapper-hidden recursive flattening, reducers or terminal predicates that ignore the flattened value, any-vs-all direction changes, Rust Iterator::all wrong empty truth, consumed iterator, callback or loop effect, mutating borrow, TypeScript/JavaScript array-param every sparse-hole boundaries, callbacks observing index/source-array arguments, JavaScript Array.prototype.every replacement, custom Java Arrays or Swift filter/allSatisfy dispatch, Swift reduce without fold proof, Swift wrong empty truth, two-argument custom allSatisfy overloads, lazy allSatisfy demand semantics, max-vs-min direction changes, unseeded selection APIs with different all-negative behavior, broad runtime no-overflow or total-order claims, and unproven overflow/float/NaN/signed-zero numeric domains remain outside the exact family through numeric.float-special-value-boundary.
- **proof invariant**: Open reduction convergence only when the compared forms traverse the same source in the same order, preserve the identity seed and empty-input behavior, satisfy numeric.aggregate-value-model-domain when controlled aggregate arithmetic matters, satisfy numeric.selection-value-order-domain when controlled min/max or relational selection matters, apply the same reducer contribution or terminal predicate to the same element coordinate, preserve any/all short-circuit direction and fallthrough result, prove receiver/API identity for protocol methods, require value-only predicate callback shape, and for min/max preserve the same explicit seed and selection domain. Flat-map aggregates additionally require one-level outer/inner traversal order, emitted-value, source-cardinality, and flatten-depth facts; distinct outer/inner coordinates even when sources alias directly or through a derived outer value; reducer steps and terminal predicates that consume the flattened element; distinct outer/inner/terminal guard coordinates; pure transform and predicate callbacks; and standard Java Arrays/Stream or Swift flatMap/filter/map/allSatisfy dispatch. Predicate/callback effects, source-independent inner maps, direct or wrapper-hidden recursive flattening, consumed iterators, mutating borrows, TypeScript/JavaScript Array.every callbacks observing index or source-array arguments, JavaScript Array.prototype.every replacement, Swift two-argument custom allSatisfy overloads, Swift lazy allSatisfy receiver demand semantics, Swift reduce without fold/operator/overload/trap proof, wrong seeds, changed contributions, guards, sources, flatten depth, terminal predicates, all-vs-any direction changes, wrong empty truth, custom aggregate dispatch, unseeded terminal selection APIs, broad runtime no-overflow or total-order claims, and unproven receiver/protocol, overflow, float, NaN, signed-zero, or numeric-domain evidence must remain non-equivalent.
- **hard negatives**:
  ...
  - Rust Iterator::all wrong empty truth, changed source, consumed iterator, callback or loop effects, and mutating borrows are outside the focused universal proof
  - TypeScript/JavaScript array parameters do not prove dense Array.every sources because sparse holes are skipped by every but observable to for-of iteration
  - TypeScript/JavaScript Array.every callbacks that observe index or source-array arguments are outside the value-only predicate proof
  - JavaScript Array.prototype.every assignment or defineProperty replacement breaks standard API identity
  ...
  - Java Stream flatMap/reduce changed identity, step, outer or inner guard, source/cardinality, source-independent inner map, aliased same/derived coordinate, direct or wrapped recursive flatten depth, ignored reducer value, callback effect, or shadowed Arrays dispatch stay split
  - Swift flatMap/allSatisfy changed outer, inner, or terminal predicate, source/cardinality, source-independent inner map, ignored terminal value, flatten depth, callback effect, custom filter dispatch, or custom allSatisfy dispatch stay split
  - Swift reduce remains outside this aggregate slice until its fold callback, operator, overload, and trap behavior are separately proven
...
- **real frontier replay**: `reduction-sum-focused-controlled-pair`, `reduction-any-focused-controlled-pair`, `reduction-rust-all-focused-controlled-pair`, `reduction-rust-all-effect-boundary-controlled-pair`, `reduction-rust-all-mutating-borrow-boundary-controlled-pair`, `reduction-typescript-every-dense-literal-controlled-pair`, `reduction-typescript-every-array-param-boundary-controlled-pair`, `reduction-javascript-every-dense-literal-controlled-pair`, `reduction-javascript-every-array-param-boundary-controlled-pair`, `reduction-typescript-every-append-only-flags-drizzle-real-pair`, `reduction-selection-focused-controlled-pair` (`real_frontier_replay.v1.json`)
- **representative locations**:
  - `nose` (focused, multi-language) `bench/type4/adversarial/cases/reduction_minmax_anyall/sum.py:1-23`
  - `nose` (focused, Rust) `bench/type4/adversarial/cases/reduction_minmax_anyall/any_all.rs:1-115`
  - `nose` (focused, TypeScript) `bench/type4/adversarial/cases/reduction_minmax_anyall/any_all.ts:1-124`
  - `nose` (focused, JavaScript) `bench/type4/adversarial/cases/reduction_minmax_anyall/any_all.js:1-99`
  - `nose` (focused, JavaScript) `bench/type4/adversarial/cases/reduction_minmax_anyall/every_patch.js:1-35`
  ...
  - `nose` (focused, Java) `bench/type4/adversarial/cases/java_flat_map_aggregate/aggregate.java:1-154`
  - `nose` (focused, Swift) `bench/type4/adversarial/cases/swift_flat_map_aggregate/aggregate.swift:1-147`
...
- **current detector result (primary linked evidence)**: miss=False · `nose` @ `#797 Java/Sw` — Semantic queries report a broad sum/reduce family, TypeScript any/some, dense-literal one-argument TypeScript and JavaScript every/for-of, Swift eager allSatisfy/loop, Rust any/all terminal families, focused Rust Iterator::all universal loop families, seeded selection families, Java Stream flatMap/reduce versus nested-loop families, and Swift flatMap/allSatisfy versus nested-counterexample-loop families. Wrong seeds, changed reduction steps, outer/inner/terminal guards, sources/cardinality, source-independent inner maps, same/derived-source coordinate aliases, direct or wrapper-hidden recursive flattening, ignored reducer/terminal value coordinates, callback effects, custom Java Arrays dispatch, custom Swift filter/allSatisfy dispatch, sparse-array and callback-extra-argument boundaries, prototype replacement, lazy receivers, consumed iterators, mutating borrows, and unseeded selection stay outside those families.
- **detector admission**: `controlled-slice-admitted` · controlled integer/value-model sum/product, any/all terminal, focused Rust Iterator::all universal loops, dense-literal TypeScript/JavaScript every, Swift eager allSatisfy, Java Stream flatMap/reduce, Swift flatMap/allSatisfy, and seeded min/max selection reductions with source, one-level flatten order/value/depth, aggregate guard coordinate, identity/empty, aggregate value-model numeric-domain, selection value-order numeric-domain, float-special-value boundary, step/predicate, short-circuit direction, selection seed/domain, receiver/API identity, and predicate/reducer effect evidence
