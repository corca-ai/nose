...
TSTree *ts_parser_parse(
  ...
  do {
    ...
    // The existing finished-tree comparison uses this as a lower bound on every
    // live parse. Include missing-token errors in the progress signal once no
    // zero-error completion remains possible.
    if (
      min_error_cost > 0 && min_error_cost < UINT_MAX &&
      (!self->finished_tree.ptr || ts_subtree_error_cost(self->finished_tree) > 0)
    ) {
      self->has_error = true;
    }
