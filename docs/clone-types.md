...
- Sub-function semantic coverage is intentionally bounded: nose extracts control-flow
  ...
  conditional return/throw guards, including explicit empty no-op branches), not arbitrary
  statement windows with unmodeled free-variable, live-out, or effect boundaries.
