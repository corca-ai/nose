...
Those fragments are reported only when the whole value subtree stays inside the displayed
source span, the fragment is `exact_safe`, and the value fingerprint is large enough for
the exact semantic gate.
