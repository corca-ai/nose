...
| `member-context=N full` | with a selected family/member, inspect nearby live-source lines; follow **Inspect surrounding code** on a member for up to 20 lines each side, within known enclosing-unit and file bounds. Reported member coordinates and evidence stay unchanged; output is bounded and marks truncation. |
