...
| `base=REF` | the **divergent-edit** view: detect families at the git ref, flag the ones a diff changed in one copy but not its siblings — a likely un-propagated fix. It is its own view, so combine it only with `top=N`, detection flags, `--format`, or `--fail-on any`; ordinary family filters are for the non-`base=` query views. Each item carries `fire_eligible` (the conservative proven-shared-logic verdict); `base=REF --fail-on any` is the current CI gate (fires only on the proven case). The v2 contract will expose explicit `strict` / `review` / `report-only` / `suppressed` tiers; see [divergent edits](divergent-edits.md). |
