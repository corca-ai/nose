...
> (literal `lo <= hi`, or asserted `Guard(BoundOrder)` evidence for a dominating
> inverse guard such as `if hi < lo { throw ... }` or a branch-local positive guard
> such as `if lo <= hi { return ... }`);
