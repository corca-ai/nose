...
- **Soundness is axis-dependent** (the two-axis principle, [experiments](experiments.md) §AH).
  ...
  **type-gated**, never assumed: `+` commutes only on non-concat operands (string/list `+`
  ...
  errors just like the loop condition and body. Eager builtin arguments, list/tuple literal
  items, higher-order map/filter/reduce, and `any`/`all` predicate errors also stay
