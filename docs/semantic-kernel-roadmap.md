...
- Immutable local/module binding domains now produce binding-anchored `Domain`
  ...
  and the first-party mutation walk finds no direct binding/place mutation.
...
- Value-graph and structural-recursion domain gates moved from normalize-local
  ...
  ordering.
- An experimental `abstraction` detection mode landed as a weak sibling claim over a
...
- Abstraction query output now requires family-wide hole agreement: every reported
...
- Add receiver/place facts so field read/write and property contracts are not
  field-name-only.
- Add provenance fields internally before exposing them in query JSON.
...
- What is the minimum provenance that query JSON must expose without making reports
  noisy?
...
- recorded intentional old-behavior changes where missing evidence blocks exact
  convergence;
- kept tests and docs checks green after the proof-gated query follow-up;
...
The follow-up [promise-imported-call-return-boundary-2026-06-29.v1.json](../bench/recall_loss/promise-imported-call-return-boundary-2026-06-29.v1.json) keeps exact admission closed but corrects the imported target-present obligation vocabulary. Imported function/member call-target evidence proves only a module/export/member coordinate; unlike DirectFunction or DirectMethod evidence, it does not provide an evaluable local body. These receivers now require a settled-value contract before Promise continuation recovery can open. The prior corpus query keeps the queue priced at `105` imported-member candidates across `9` repos and `73` imported-binding candidates across `15` repos.
...
2026-07-06 #677 Rust block_on residual closeout note:
The [#677 Rust block_on residual closeout](../bench/recall_loss/issue-677-rust-block-on-residual-closeout-2026-07-06.v1.json) takes one of the #657 follow-up rows through the recovery decision loop. The selected tranche stays reporting-only: existing Rust runtime-provenance rows can attribute future-drive obligations, but exact `block_on` convergence still lacks nested runtime scheduling/liveness proof, callable result provenance for wrapper-returned runtimes, field lifecycle and mutation proof for constructor-assigned runtime fields, and result-channel contracts for `block_on` versus `await` or returning a future. Those exact-recovery prerequisites are tracked by [#680](https://github.com/corca-ai/nose/issues/680). The executable suite now adds three targeted hard negatives for those missing proofs. Product code and exact admission are unchanged; the `crates` gate remains at `0` false merges and `0` canon-preservation violations, focused Rust query-regression hashes stay identical, and runtime triage reproduces no degradation.
